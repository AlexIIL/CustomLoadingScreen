...
import alexiil.mc.mod.load.expression.api.*;
...
import alexiil.mc.mod.load.expression.node.arg.NodeArgumentDouble;
import alexiil.mc.mod.load.expression.node.arg.NodeArgumentLong;
import alexiil.mc.mod.load.expression.node.binary.*;
import alexiil.mc.mod.load.expression.node.cast.NodeCastBooleanToString;
import alexiil.mc.mod.load.expression.node.cast.NodeCastDoubleToString;
import alexiil.mc.mod.load.expression.node.cast.NodeCastLongToDouble;
import alexiil.mc.mod.load.expression.node.cast.NodeCastLongToString;
import alexiil.mc.mod.load.expression.node.condition.NodeConditionalBoolean;
import alexiil.mc.mod.load.expression.node.condition.NodeConditionalDouble;
import alexiil.mc.mod.load.expression.node.condition.NodeConditionalLong;
import alexiil.mc.mod.load.expression.node.condition.NodeConditionalString;
import alexiil.mc.mod.load.expression.node.unary.NodeBooleanInvert;
import alexiil.mc.mod.load.expression.node.unary.NodeUnaryDouble;
import alexiil.mc.mod.load.expression.node.unary.NodeUnaryLong;
...
class InternalCompiler {
    ...
    private static final String HEX_REGEX = "0x[0-9a-f]+";
    ...
    private static final Pattern HEX_MATCHER = Pattern.compile(HEX_REGEX);
    ...
    public static Pair<IExpressionNode, ArgumentCounts> compileExpression(String expression, FunctionContext context) throws InvalidExpressionException {
        try {
            ...
            throw new InvalidExpressionException("Invalid expression (" + expression + ")", iee);
        }
    }
    ...
    private static Pair<IExpressionNode, ArgumentCounts> makeExpression(Argument[] args, String[] postfix, FunctionContext context) throws InvalidExpressionException {
        ...
        for (String op : postfix) {
            ...
            } else if (HEX_MATCHER.matcher(op).matches()) {
                stack.push(new NodeImmutableLong(Long.parseLong(op.substring(2), 16)));
