...
import alexiil.mc.mod.load.expression.Tokeniser.*;
...
public class TokeniserDefaults {
    ...
    public static final ITokenizerGobbler GOBBLER_NUMBER_HEX = (ctx) -> {
        if (!ctx.get(2).equals("0x")) {
            return ResultSpecific.IGNORE;
        }
        int i = 2;
        while (true) {
        ...
        return i == 2 ? ResultSpecific.IGNORE : new ResultConsume(i);
    };
